...
'''
Nguồn: https://github.com/skanelo/Face-Mask-Detection/blob/main/show_bb.py
'''
	...
	for image_file in glob.glob(dir_path + '/*.png'):
		...
		with open(image_file.split(".")[0] + '.txt', 'r') as reader:
			...
			for annot in annotations:
				...
				x1, y1 = int((float(annot[1]) - float(annot[3]) / 2) * width), int(
					(float(annot[2]) - float(annot[4]) / 2) * height)
				x2, y2 = int((float(annot[1]) + float(annot[3]) / 2) * width), int(
					(float(annot[2]) + float(annot[4]) / 2) * height)
